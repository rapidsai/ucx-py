...
def talk_to_client(client_ep):
    ...
    for i in range(msg_log):
        ...
        for j in range(warmup_iters):
            ...
            send_req = comm_ep.send_fast(send_msg, msg_len)
            recv_req = comm_ep.recv_fast(recv_msg, msg_len)
            send_req.result()
            recv_req.result()
        ...
        for j in range(iters):
            ...
            tmp_start = time.time()
            send_req = comm_ep.send_fast(send_msg[j], msg_len)
            tmp_end = time.time()
            ...
            tmp_start = time.time()
            recv_req = comm_ep.recv_fast(recv_msg[j], msg_len)
            tmp_end = time.time()
            issue_lat += (tmp_end - tmp_start)

...
class EndpointReuse:
    ...
    async def create_endpoint(cls, ip, port):
        ...
        for ep in existing_endpoints:
            if not ep.ep.closed():
                ep.refcount += 1
                my_ep_ids.append(ep.ep._tags["msg_tag_recv"])
        ...
        if reuse_ep_id:
            ...
            assert ep_new._tags["msg_tag_send"] not in cls.existing_endpoints
            cls.existing_endpoints[ep_new._tags["msg_tag_send"]] = reuse_ep
    ...
    def create_listener(cls, cb_coroutine, port):
        async def _handle(ep_new):
            ...
            if existing_ep:
                ...
                assert ep_new._tags["msg_tag_send"] not in cls.existing_endpoints
                cls.existing_endpoints[ep_new._tags["msg_tag_send"]] = existing_ep
